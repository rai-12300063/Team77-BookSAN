    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.resetUrl) {
                setMessage(`${response.data.message}. Reset URL: ${response.data.resetUrl}`);
            }
        } catch (error) {
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div>
                        <button
                            type="submit"
                            disabled={loading}
                            className="font-medium w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
